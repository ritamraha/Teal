...
By default, this will run *TEAL* on `example.trace` with a future-reach bound of 2.  
...
|Argument        |Meaning
|----------------|------------------------------
|-i <input_sample>| For specifying the input sample; default is *example.trace*.
|-f <fr_bound>| For specifying the future-reach bound of the prospective formula
|-m | For specifying whether the prospective formula should be *globally* separating or only separating
|-t <timeout>| For specifying the timeout
|-o <outputcsv>| For specifying the csv file with the output results
...
|-a | For specifying whether all experiments from the research question should be run; by default, it only runs on a subset.
|-t <timeout> | For specifying the timeout for each experiment
...
The singals in the sample file are piecewise-constant signals.
...
An example of a trace is `0.0:1,0;1.2:1,1` which consists of two timepoints 0.0 and 1.2 and in the first timepoint only `p` holds,
